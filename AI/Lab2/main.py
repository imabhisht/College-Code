class SolveJugProblem:
    ...
    def start_the_search(self):
        ...
        while queue:
            current_selected, path = queue.pop(0)
            ...
            neighbour_list = []
            neighbour_list.append((self.op_FILL_JUG1(current_selected), "Fill Jug 1"))
            neighbour_list.append((self.op_FILL_JUG2(current_selected), "Fill Jug 2"))
            neighbour_list.append((self.op_EMPTY_JUG1(current_selected), "Empty Jug 1"))
            neighbour_list.append((self.op_EMPTY_JUG2(current_selected), "Empty Jug 2"))
            neighbour_list.append((self.op_POUR_JUG1_TO_JUG2(current_selected), "Pour Jug 1 to Jug 2"))
            neighbour_list.append((self.op_POUR_JUG2_TO_JUG1(current_selected), "Pour Jug 2 to Jug 1"))

            for neighbour, operation in neighbour_list:
                if neighbour not in graph_is_visited:
                    graph_is_visited[neighbour] = True
                    queue.append((neighbour, path + [operation]))
